    ...
    <div className="space-container">
      <ul aria-live="polite" className="mb-2 w-full ">
        {prompts.map((prompt, index) => (
          <li key={index} className="bg-background text-foreground mb-2 rounded">
            <Button
              onClick={() => handlePromptClick(prompt)}
              className={`text-buttonText w-full rounded p-2 text-center ${selectedPrompt === prompt ? "bg-button" : "text-buttonText"}`}
